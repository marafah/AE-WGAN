...
def AE_WGAN():
    ...
    class AE_WGAN():
        ...
        def build_generator(self):
            ...
            model.add(Dense(20, input_dim=self.latent_dim, activation='linear'))
        ...
        def build_discriminator(self):
            ...
            model_ = Sequential()
            model_.add(Dense(80, input_dim=self.latent_dim, activation='linear'))
            model_.add(Dense(40, activation='linear'))
            ...
            model = Dense(80, activation='linear')(d_in)
        ...
        def train(self, attack_name, epochs, batch_size=32):
            ...
            plt.savefig('.\\Datasets\\CICIDS\\generated\\AE_WGAN\\plots\\E_AE_WGAN_selectedAttacks_%s.jpeg' % attack_name, bbox_inches='tight')
            plt.show()
            
            cwd = os.getcwd()
            print(cwd + '\\Datasets\\CICIDS\\generated\\AE_WGAN\\plots\\')
    ...
    all_attacks = ['BENIGN', 'Bot', 'PortScan', 'DDoS', 'Brute Force', 'FTP-Patator',
                   'SSH-Patator', 'DoS Slowhttptest', 'DoS Hulk', 'DoS slowloris',
                   'DoS GoldenEye', 'XSS', 'Infiltration', 'Sql Injection', 'Heartbleed']

    #This is the starting point of the code
    for attack_name in all_attacks:
    
        #Test data splited into normal and attack
        attack = pd.read_csv('.\\Datasets\\CICIDS\\Preprocessing_data\\attack names 15 categories\\%s.csv' % attack_name)
        
        #Spliting the normal data into features and label
        y_attack=attack.iloc[:,[-1]]
        X_attack=attack.drop(y_attack.columns,axis = 1)
        
        #Reshaping the normal and attack data as 2x5 images to be used in AE_WGAN
        reshaped_X_attack = np.asarray(X_attack).reshape(-1, 2, 5, 1)
        ...
        generated_attack = generated_attack.reshape(-1,10)
        ...
        q.to_csv('.\\Datasets\\CICIDS\\generated\\AE_WGAN\\generated attacks\\%s.csv' % attack_name, index=False)
    ...
    for x in all_attacks:
        ...
        generated = pd.read_csv('.\\Datasets\\CICIDS\\generated\\AE_WGAN\\generated attacks\\%s.csv' % x)
        
        df = pd.concat([df, generated])
    
    df.to_csv('.\\Datasets\\CICIDS\\generated\\AE_WGAN\\generated attacks\\all_attacks.csv', index=False)
